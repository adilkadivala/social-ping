...
export class MonitoringService {
  ...
  private async monitorKeyword(keyword: any): Promise<void> {
    ...
    try {
      console.log(`🔍 Monitoring keyword: "${keyword.keyword}" for user: ${keyword.user.email}`)
      ...
      if (newMentions.length > 0) {
        console.log(`✅ Found ${newMentions.length} new mentions for "${keyword.keyword}"`)
      }
  ...
  private analyzeSentiment(text: string): 'positive' | 'negative' | 'neutral' {
    ...
    const negativeWords = ['bad', 'terrible', 'awful', 'hate', 'worst', 'horrible', 'sucks', 'disappointing']
