...
router.get(
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const twitter =
        platformStats?.filter((m) => m.platform === "twitter").length || 0;
      const reddit =
        platformStats?.filter((m) => m.platform === "reddit").length || 0;
      const linkedin =
        platformStats?.filter((m) => m.platform === "linkedin").length || 0;
      const youtube =
        platformStats?.filter((m) => m.platform === "youtube").length || 0;
